...
We want to get each subject's cortical Surface Area and Thickness Average of each region of interst (ROI) after running FreeSurfer. In this first step, extract and organize each of the values for each FreeSurfer ROI. The script assumes that your FreeSurfer output are organized in a standard way:
_/enigma/Parent_Folder/FreeSurfer/outputs/subject1/_

SCRIPT: `extract_cortical_measures_v2021.sh` 

_NB: this is an updated script version which correctly extracts the full ICV number with %f to avoid rounding up of the values when the CSV is opened and edited._
...
Run script: `sh extract_cortical_FreeSurfer_measures_v2021.sh`
...
Run script: `sh make_cortical_FreeSurfer_internal_QC_png_slices.sh`
...
Run script: `sh make_cortical_FreeSurfer_external_QC_png_v2021.sh`
...
Run in your terminal or R window: `R --no-save --slave < outliers_cortical.R > /enigma/Parent_Folder/FreeSurfer/measures/outliers_cortical.log `
