...
function make_pngsFSL(dirO,subject,imageF,overlay)
    ...
    if (length(inputFAimg(:))==Nx*Ny*Nz && length(inputFIBimg(:))==Nx*Ny*Nz )
	    ...
	    %set(gcf,'Visible','off');
            ...
            C2 = min(m,round((m-1)*(squeeze(inputFIBimg(sliceS,:,:))-cminF)/ ...
                (cmaxF-cminF))+1)+64;
            h1(1)=image(Ymin-2:Ymax+2,Zmax+2:-1:Zmin-2,transpose(squeeze(inputFAimg(sliceS,:,:)))) ;hold on;axis off; axis square;
            set(h1(1),'CData',C1');
        ...
        for struct = 1:2:14
            ...
            for slice = 1:3
                ...
                %%%%%%%%%%%%%%%%%
                C1 = min(m,round((m-1)*(squeeze(inputFAimg(:,sliceC,:))-cmin)/(cmax-cmin))+1);
                C2 = min(m,round((m-1)*(squeeze(structures(:,sliceC,:))-cminF)/ ...
                    (cmaxF-cminF))+1)+64;
                ...
                C2 = min(m,round((m-1)*(squeeze(inputFIBimg(sliceS,:,:))-cminF)/ ...
                    (cmaxF-cminF))+1)+64;
                h(1)=image(Ymin-2:Ymax+2,Zmax+2:-1:Zmin-2,transpose(squeeze(inputFAimg(sliceS,:,:)))) ;hold on;axis off; axis square;
                set(h(1),'CData',C1');
