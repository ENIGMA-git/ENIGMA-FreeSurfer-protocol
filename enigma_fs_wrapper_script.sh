...
Mandatory arguments:
	...
	--matlab		Matlab path
	--fs_version	Freesurfer version (choose from 5, 6, 7 ,or 8)
...
if [[ "${step_val_1}" = true ]]; then 
	...
	echo "SubjID,LLatVent,RLatVent,Lthal,Rthal,Lcaud,Rcaud,Lput,Rput,Lpal,Rpal,Lhippo,Rhippo,Lamyg,Ramyg,Laccumb,Raccumb,ICV,Freesurfer_Version" > ${measures_dir}/LandRvolumes.csv

	# setting default to Freesurfer v7 if version not specified:
	case "${fs_version}" in 
		5|6|7|8)
			echo "running for Freesurfer version ${fs_version}"
		;;
		*)
			echo "version not found, setting to default Freesurfer 7"
			fs_version=7
		;;
	esac


	if [[ "${fs_version}" -ge 7 ]]; then
		for subj_id in `cat $subjectIDs`; do
			...
			printf ",%s" $(grep -oE "[0-100]+\.[0-100]+\.[0-100]+" ${fs_dir}/${subj_id}/scripts/build-stamp.txt) >> ${measures_dir}/LandRvolumes.csv
...
if [[ "${step_val_2}" = true ]]; then
	...
	for subj_id in `cat $subjectIDs`; do
		...
		printf ",%s" $(grep -oE "[0-100]+\.[0-100]+\.[0-100]+" ${fs_dir}/${subj_id}/scripts/build-stamp.txt) >> ${measures_dir}/CorticalMeasuresENIGMA_ThickAvg.csv
		printf ",%s" $(grep -oE "[0-100]+\.[0-100]+\.[0-100]+" ${fs_dir}/${subj_id}/scripts/build-stamp.txt) >> ${measures_dir}/CorticalMeasuresENIGMA_SurfAvg.csv
...
if [[ "${step_val_3}" = true ]]; then
	...
	for subj_id in `cat $subjectIDs`; do
		if [[ "${fs_version}" = 8 ]]; then
			matlabcall="addpath(genpath('${scripts_dir}')); func_make_subcorticalFS_ENIGMA_QC('${qc_dir_subcort}','${subj_id}','${fs_dir}/${subj_id}/mri/nu.mgz','${fs_dir}/${subj_id}/mri/aparc+aseg.mgz')"
		else
...
if [[ "${step_val_4}" = true ]]; then
	...
	for subj_id in `cat $subjectIDs`; do
		if [[ "${fs_version} = 8" ]]; then
			matlabcall="addpath(genpath('${scripts_dir}')); func_make_corticalpngs_ENIGMA_QC('${qc_dir_cort_int}','${subj_id}','${fs_dir}/${subj_id}/mri/nu.mgz','${fs_dir}/${subj_id}/mri/aparc+aseg.mgz')"
		else
